...
AIARC는 **문서·도면·메일·사진과 변경 기록을 연결해, 건축 프로젝트의 현재 상태와 다음에 할 일을 파악하도록 돕는 AI 업무공간**을 만들고 있습니다. 이 제품을 **AI Project Workspace**라고 부릅니다.

첫 적용 분야는 **공사를 마무리하는 준공 단계의 문서 업무**입니다. 건축사사무소 등에서 반복하는 자료 취합·정리·누락 확인·보완 요청을 줄이는 Windows 데스크톱 앱으로 시작합니다.

**현재 공개된 0.6.0은 가상 자료로 제품 흐름을 체험하는 프로토타입입니다.** 파일 정보 확인과 문서 열기는 실제로 동작하지만, 분류·누락 분석·보완 요청·회신은 준비된 데모 데이터로 시연합니다. 실제 AI 분석이나 메일 송수신 기능이 완성된 서비스는 아닙니다.

이 저장소에서 사용하는 주요 개념은 다음과 같습니다.
...
핵심 능력     Project State
사업 확장     건설 AI 업무 인프라
장기 목표     Architecture AI
```

**Project State는 프로젝트의 현재 상태를 뜻합니다.** AIARC는 어떤 자료가 최신인지, 무엇이 바뀌었는지, 아직 확인할 일이 무엇인지, 그 근거가 어디에 있는지를 함께 파악하려 합니다. 별도 제품 단계가 아니라 AI 업무공간 전반에서 활용할 핵심 능력입니다.
...
AIARC는 개발자가 건축사사무소에서 건축사보로 근무하며 감리·사용승인 업무에 참여한 경험에서 출발했습니다. 자료 확인, 누락 자료 요청, 수정·보완본 반영을 반복하며 겪은 어려움을 첫 제품의 문제로 삼았습니다.
...
현재 0.6.0 프로토타입은 여러 곳에서 받은 준공 자료를 정리하고, 누락되거나 내용이 맞지 않아 사람이 확인해야 할 항목을 보여줍니다. 아래 흐름의 분석 결과는 준비된 샘플을 사용합니다.
...
준공 단계에서 문서 이해와 관계 연결 능력을 검증한 뒤, 이를 공사가 진행되는 동안에도 활용하는 **AI Project Workspace**로 발전시키려 합니다. 자료가 들어올 때마다 관련 문서와 변경 기록을 연결해 프로젝트의 현재 상태를 파악하는 것이 목표입니다.
...
Project State의 상세 사례와 제품 원리는 [제품 개요](./docs/PRODUCT_OVERVIEW.md)에서 설명합니다.
...
감리·발주·인허가·사용승인 등 기존 제도와 시스템을 대체한다는 뜻이 아닙니다. **정리한 자료와 처리한 업무가 기존 외부 업무·행정 시스템으로 이어지는 AI 업무공간**을 지향합니다.

> **장기적으로는 사람과 기업 사이의 자료·요청·변경 기록을 연결하는 건설 AI 업무 인프라로 발전하려 합니다.**

---

## 제품 설계 원칙

AIARC는 **자료 정리를 자동화하고, 사용자는 확인이 필요한 항목에 집중하는 Windows 데스크톱 작업공간**을 지향합니다. 다음은 제품이 구현하려는 업무 흐름입니다.

```text
PC에 저장된 파일 / 현장자료 폴더 / ZIP 가져오기
→ AIARC 자동 분석
→ 문제가 없는 문서는 자동 처리
→ 누락 · 내용 불일치 · 판단이 필요한 항목 표시
...
핵심 능력
Project State
└ 프로젝트의 현재 상태 · 변경 · 확인이 필요한 일 · 근거를 지속적으로 구축하고 활용
