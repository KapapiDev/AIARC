...
초기 제품은 회사 전체의 시스템 교체나 여러 참여자의 동시 도입을 전제로 하지 않습니다. 기존처럼 **이메일로 개별 파일이나 ZIP을 주고받고 PC의 현장별 폴더에 저장·관리하는 업무환경을 바꾸지 않아도**, 사용자 한 명이 먼저 가치를 얻을 수 있어야 합니다.
...
현재 건설 업무에서는 **이메일과 전화로 자료와 요청을 주고받고, HWP·PDF·Excel 등 다양한 파일을 각자의 PC와 현장 폴더에 저장·관리하며 여러 외부 시스템을 함께 사용합니다.** AIARC는 이 도구와 방식을 모두 대체하려는 것이 아니라, **그 사이에 흩어진 프로젝트 자료와 업무의 맥락을 연결하는 방향**으로 확장합니다.
...
- 기존처럼 이메일로 자료를 주고받고, HWP·PDF·Excel 등을 PC의 현장 폴더에 저장·관리하는 Windows 업무환경을 존중합니다.
